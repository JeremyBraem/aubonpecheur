<div id="update-appatModal-<?php echo $appat->getIdProduit() ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-2xl max-h-full">
        <!-- Modal content -->
        <div class="relative p-4 bg-white rounded-lg shadow">
            <!-- Modal header -->
            <div class="flex justify-between items-center pb-4 mb-4 rounded-t border-b sm:mb-5">
                <h3 class="text-lg font-semibold text-gray-900 ">Modifier une appat</h3>
                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center" data-modal-target="update-appatModal-<?php echo $appat->getIdProduit() ?>" data-modal-toggle="update-appatModal-<?php echo $appat->getIdProduit() ?>">
            ...
            <form action="admin.php?action=updateAppatTraitement" method="post" enctype="multipart/form-data">
                ...
                <div class="grid gap-4 mb-4 sm:grid-cols-2">
                    ...
                    <div>
                        <label for="nom" class="block mb-2 text-sm font-medium text-gray-900">Nom de l'appât</label>
                        <input type="text" value="<?php echo $appat->getNomProduit(); ?>" name="nom_produit" id="nom_produit" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Écrire le nom de l'article" required>
                    </div>
